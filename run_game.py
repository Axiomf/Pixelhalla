...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
            break
        elif event.type == BOSS_LLM_REPLY_EVENT:# --- Handle Boss LLM Reply Event ---
    ...
    if dialog_initialized and not is_waiting_for_boss_llm and not is_waiting_for_fighter_llm and not is_delaying_for_next_turn:
